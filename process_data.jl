...
sc = scatter([p.density for p in planets], [p.stellar_temp for p in planets], c=[p.distance for p in planets])
cbar = colorbar(sc)
axis([0, 30, 2500, 6500])
title("Super Earth Exoplanets Under 10 Earth Masses")
cbar[:ax][:set_ylabel]("Orbital Radius [AU] (-1=no data)")
xlabel("Planetary Density [g/cm3]")
